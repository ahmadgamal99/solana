...
pub mod tests {
    use {
        super::*,
        solana_sdk::{account::Account, native_token::rox_to_lamports},
    };
    ...
    fn test_calculate_stake_weighted_timestamp_uses_median() {
        ...
        let stakes: HashMap<Pubkey, (u64, Account)> = [
            (
                pubkey0,
                (rox_to_lamports(1.0), Account::new(1, 0, &Pubkey::default())),
            ),
            (
                pubkey1,
                (rox_to_lamports(1.0), Account::new(1, 0, &Pubkey::default())),
        ...
        let stakes: HashMap<Pubkey, (u64, Account)> = [
            (
                pubkey0,
                (
                    rox_to_lamports(1_000_000.0), // 1/3 stake
                    Account::new(1, 0, &Pubkey::default()),
                ),
            ),
            (
        ...
        let stakes: HashMap<Pubkey, (u64, Account)> = [
            (
                pubkey0,
                (
                    rox_to_lamports(1_000_001.0), // 1/3 stake
                    Account::new(1, 0, &Pubkey::default()),
                ),
            ),
            (
