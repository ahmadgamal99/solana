...
/// The address of a [Rox account][acc].
///
/// Some account addresses are [ed25519] public keys, with corresponding secret
/// keys that are managed off-chain. Often, though, account addresses do not
/// have corresponding secret keys &mdash; as with [_program derived
/// addresses_][pdas] &mdash; or the secret key is not relevant to the operation
/// of a program, and may have even been disposed of. As running Rox programs
...
impl Pubkey {
    ...
    /// as a Rox `Pubkey`, except they are ensured to not be on the ed25519
    ...
    ///
    /// As all account addresses accessed by an on-chain Rox program must be
    ...
    /// program, which invokes an on-chain Rox program that uses the address
    ...
    /// instruction as a Rox transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain Rox program:
    ///
    /// ```
