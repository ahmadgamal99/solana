...
//! [`ecrecover`] precompile. For that Rox provides the [`secp256k1_recover`]
//! syscall.
//!
...
//! and callers should ensure they use the same version of `bincode` as the Rox SDK.
//! This data structure is not provided to Rox programs,
...
//! does prevent malleability by default. Rox accepts signatures with `S`
...
//! to parse the secp256k1 instruction data from within a Rox program.
//!
//! ```no_run
//! mod secp256k1_defs {
...
//!
//! This example has two components: a Rox program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the Rox program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The Rox program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//! containing many signatures, and a Rox program that parses them all. This
...
//! transaction. In the _previous_ example, the Rox program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the Rox program supports signatures, messages, and
...
///
/// `feature_set` is the set of active Rox features. It is used to enable or
